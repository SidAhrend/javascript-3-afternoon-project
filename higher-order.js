...
//Code Here
let evenNumbers = mixedNumbers.filter(item => {return item % 2 === 0;});
...
//Code Here
let postTaxPrices = prices.map(item => {
  return item * 1.07;
});
...
//Code Here
let totalPopulation = populations.reduce((total, item) => {
  return total + item;
});
...
let bobsTotal = purchases.reduce((total, item) =>{
  return item.owner === "Bob" ? total += item.price : total;
}, 0)
